...
export class DerivSession {
  ...
  handleMessage(data) {
    ...
    if (data.msg_type === "proposal_open_contract" && data.proposal_open_contract?.is_sold) {
      ...
      const settledTrade = {
        ...
        timestamp: this.state.lastTradeMeta?.timestamp || Date.now(),
        timestampBucket: this.state.lastTradeMeta?.timestampBucket || "unknown",
        marketCondition: this.state.lastTradeMeta?.marketCondition || "unknown",
        edgeState: this.state.lastTradeMeta?.edgeState || "unknown",
        analyzerStats: this.state.lastTradeMeta?.analyzerStats || {},
  ...
  placeTrade(signal) {
    ...
    const timestampBucket = hour < 8 ? "asia" : hour < 16 ? "europe" : "us";
    const marketCondition = this.state.botMode === "digits"
      ? (this.state.digitStats?.signal === "NO TRADE" ? "neutral" : "digit-bias")
      : (String(this.state.lastSignalText || "").includes("WAIT") ? "chop" : "trend");
    ...
    this.state.lastTradeMeta = {
      ...
      timestampBucket,
      marketCondition,
      edgeState: this.state.digitStats?.signal || this.state.lastSignalText,
      analyzerStats: {
        sampleSize: this.state.digitStats?.sampleSize || 0,
        evenCount: this.state.digitStats?.evenCount || 0,
        oddCount: this.state.digitStats?.oddCount || 0,
        overUnderEdge: this.state.digitStats?.overUnderEdge || 0
      },
      direction: signal,
      contractType,
      stake: stakeUsed,
